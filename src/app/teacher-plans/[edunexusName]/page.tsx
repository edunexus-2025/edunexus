...
import { AppConfig, Routes, escapeForPbFilter, studentPlansData } from '@/lib/constants';
import { Input } from '@/components/ui/input';
import {
  AlertCircle, GraduationCap, BarChart2, Users, Star, TrendingUp, ShoppingCart, ListChecks, DollarSign, Loader2, Tag, CheckCircle, ExternalLink, BookOpenCheck
} from 'lucide-react';
...
export default function TeacherPublicPlansPage() {
  ...
  const fetchData = useCallback(async (isMountedGetter: () => boolean = () => true) => {
    ...
    try {
      ...
      if (!isMountedGetter()) return;
      setPageData({ teacherData, contentPlans, teacherAvatarUrl });
  ...
  const handleSubscribeToTeacherPlan = async (plan: TeacherPlanType) => {
    ...
    if (amountForApi <= 0) { // Handle "free" plans directly
      ...
      try {
        ...
        router.push(Routes.myTeacherPortal);
      } catch(err: any) {
