...
interface TeacherQuestionRecord extends RecordModel {
  id: string;
  QuestionText?: string;
  QuestionImage?: string | null; // This is a URL
  OptionAText?: string;
  ...
  subject?: string;
  marks?: number; // This might not be in teacher_question_data, default to 1
  LessonName?: string; // Added for context
  QBExam?: string; // Added for context
}
...
export default function StudentTeacherTestLivePage() {
  ...
  const fetchTestDataAndDecideStage = useCallback(async (isMountedGetter: () => boolean) => {
    ...
    if (isAuthLoading) { if (isMountedGetter()) setIsLoadingPageData(true); return; } // Wait for auth to resolve
    ...
    try {
      const fetchedTest = await pb.collection('teacher_tests').getOne<TeacherTestDetailsRecord>(testId, { expand: 'teacherId' });
      if (!isMountedGetter()) return;
      console.log("[LivePage] Fetched test_tests record:", fetchedTest);
      ...
      if (fetchedTest.Admin_Password && !pinIsVerifiedInSession) {
      ...
      if (isMountedGetter()) {
        ...
        console.error("[LivePage] Error fetching test_tests record:", clientError.data || clientError);
      }
    } finally {
      if (isMountedGetter()) setIsLoadingPageData(false);
    }
  }, [testId, user?.id, isAuthLoading, router, toast]);
  ...
  const loadQuestions = useCallback(async (isMountedGetter: () => boolean) => {
    if (!testDetails || !testDetails.questions || testDetails.questions.length === 0) {
      if (isMountedGetter()) {
        setError("No questions are associated with this test, or teacher ID is missing from test details.");
        setTestSessionState('terminated'); // If no questions, test cannot proceed
        setIsLoadingPageData(false); // Ensure loading stops
      }
      console.warn("[LivePage] loadQuestions called but testDetails.questions is empty or testDetails.teacherId is missing. TestDetails:", testDetails);
      return;
    }
    if (!testDetails.teacherId) {
      if (isMountedGetter()) {
        setError("Teacher ID is missing from the test details. Cannot load questions.");
        setTestSessionState('terminated');
        setIsLoadingPageData(false);
      }
      console.warn("[LivePage] loadQuestions: testDetails.teacherId is missing.");
      return;
    }

    if (isMountedGetter()) setIsLoadingPageData(true); // Using isLoadingPageData as questions are part of page setup
    console.log("[LivePage] loadQuestions: Attempting to fetch questions for test. Question IDs:", testDetails.questions);

    try {
      const questionIds = testDetails.questions;
      const questionRecordsPromises = questionIds.map((id: string) =>
        pb.collection('teacher_question_data').getOne<TeacherQuestionRecord>(id, { '$autoCancel': false })
          .catch(err => {
            console.warn(`[LivePage] Failed to fetch question ${id} from teacher_question_data:`, err.data || err.message);
            return null; // Return null if a specific question fetch fails
          })
      );
      
      const fetchedQuestionsFromDb = (await Promise.all(questionRecordsPromises)).filter(q => q !== null) as TeacherQuestionRecord[];
      if (!isMountedGetter()) return;

      console.log(`[LivePage] Fetched ${fetchedQuestionsFromDb.length} questions from teacher_question_data out of ${questionIds.length} IDs.`);

      if (fetchedQuestionsFromDb.length === 0 && questionIds.length > 0) {
          const filterDebug = `IDs: ${questionIds.join(', ')}. Checked against teacher_question_data.`;
          setError(`Could not load questions for this test. This might be due to API rules on 'teacher_question_data' preventing access, or data mismatch. Please contact your teacher. Filter Info: ${filterDebug}`);
          setQuestions([]);
          setTestSessionState('terminated');
          return;
      } else if (fetchedQuestionsFromDb.length === 0 && questionIds.length === 0){
          setError("This test currently has no questions. Please contact your teacher.");
          setQuestions([]);
          setTestSessionState('terminated');
          return;
      }
      ...
      const questionsWithUrls = fetchedQuestionsFromDb.map(q => ({
        ...q,
        QuestionText: q.QuestionText, // Ensure these are correctly mapped from source
        ...
        marks: typeof q.marks === 'number' ? q.marks : 1, // Default marks
      }));
      ...
      console.log("[LivePage] Questions loaded successfully. State set to 'inProgress'.");

    } catch (err: any) {
      if (isMountedGetter()) {
        const clientError = err as PocketBaseClientResponseError;
        setError(`Critical error loading questions: ${clientError.data?.message || clientError.message}`);
        setTestSessionState('terminated');
  ...
  }, [testSessionState, timeLeft]); // Removed handleSubmitTest to prevent re-triggering submission
  ...
  const handleStartTestAfterInstructions = () => {
    ...
    setTestSessionState('inProgress'); // This will trigger loadQuestions via useEffect
    loadQuestions(() => true); // Directly call loadQuestions
  };
  
  const handleSubmitTest = useCallback(async (autoSubmit = false, terminationReason?: string) => {
    // ... (submission logic remains largely the same as previously provided) ...
    // Ensure it saves to 'teacher_test_attempts' and uses correct field names.
     if (!user || !testDetails || !testDetails.expand?.teacherId || isSubmittingTest || testStatus === 'completed' || testStatus === 'terminated') {
    ...
    const answersLog = questions.map(q => {
      ...
      // CorrectOption from teacher_question_data is "Option A", "Option B" etc.
      const questionCorrectOptionValue = q.CorrectOption;
      const questionMarks = typeof q.marks === 'number' ? q.marks : 1;
      
      if (selected) { 
        attemptedCount++; 
        if (selected === questionCorrectOptionValue) { 
          correctCount++; 
          isCorrectAns = true; 
          pointsEarnedFromTest += questionMarks;
        } 
      }
      return { questionId: q.id, selectedOption: selected, correctOption: questionCorrectOptionValue || null, isCorrect: isCorrectAns, markedForReview: userAnswerRec?.markedForReview || false, timeSpentSeconds: userAnswerRec?.timeSpentSeconds || 0 };
    });
    ...
    const resultDataToSave: Partial<TeacherTestAttempt> = {
      ...
      teacher: testDetails.teacherId, // Use teacherId directly from testDetails
      ...
      plan_details: { studentPlan: user.studentSubscriptionTier || 'Free', teacherPlanId: undefined }
    };
  ...
  }, [user, testDetails, questions, userAnswers, timeLeft, router, toast, isSubmittingTest, testSessionState, currentQuestion]); // Added currentQuestion
  ...
  const handleClearResponse = () => { if (testSessionState !== 'in_progress' || !currentQuestion) return; setUserAnswers(prev => ({ ...prev, [currentQuestion.id]: { ...(prev[currentQuestion.id] || { questionId: currentQuestion.id, timeSpentSeconds: 0 }), selectedOption: null, isCorrect: null, }})); };
  const handleMarkForReview = () => { if (testSessionState !== 'in_progress' || !currentQuestion) return; setUserAnswers(prev => ({ ...prev, [currentQuestion.id]: { ...(prev[currentQuestion.id] || { questionId: currentQuestion.id, selectedOption: null, isCorrect: null, timeSpentSeconds: 0 }), markedForReview: !prev[currentQuestion.id]?.markedForReview, }})); };
  const navigateQuestion = (directionOrIndex: 'next' | 'prev' | number) => { if (testSessionState !== 'in_progress' || !currentQuestion) return; if (userAnswers[currentQuestion.id]) { const currentTime = Date.now(); const timeSpentCurrentQuestion = Math.round((currentTime - questionStartTimeRef.current) / 1000); setUserAnswers(prev => ({ ...prev, [currentQuestion.id]: { ...prev[currentQuestion.id], timeSpentSeconds: (prev[currentQuestion.id].timeSpentSeconds || 0) + timeSpentCurrentQuestion }})); } let newIndex = currentQuestionIndex; if (directionOrIndex === 'next') newIndex = Math.min(questions.length - 1, currentQuestionIndex + 1); else if (directionOrIndex === 'prev') newIndex = Math.max(0, currentQuestionIndex - 1); else if (typeof directionOrIndex === 'number') newIndex = Math.max(0, Math.min(questions.length - 1, directionOrIndex)); setCurrentQuestionIndex(newIndex); questionStartTimeRef.current = Date.now(); };
  ...
  if (testSessionState === 'instructions') { return ( <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/80 p-4"> <Card className="w-full max-w-2xl shadow-xl bg-card text-foreground"> <CardHeader><CardTitle className="text-2xl">Test Instructions: {testDetails?.testName}</CardTitle><CardDescription>From: {teacherName}. Read carefully.</CardDescription></CardHeader> <CardContent className="max-h-[60vh] overflow-y-auto prose prose-sm dark:prose-invert"> <p>Total Questions: {questions.length || testDetails?.questions?.length || 'N/A'}</p> <p>Duration: {testDetails?.duration || 'N/A'} minutes</p> <p>General test instructions will be displayed here. For now, please ensure you are ready to begin.</p> </CardContent> <CardFooter className="justify-center"> <Button onClick={handleStartTestAfterInstructions} size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground">I'm Ready, Start Test!</Button> </CardFooter> </Card> </div> ); }
  ...
  if (!currentQuestion && testSessionState === 'inProgress') { return ( <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/90 p-4 text-white"> <Card className="w-full max-w-lg text-center shadow-xl bg-background text-foreground"> <CardHeader> <AlertCircle className="mx-auto h-12 w-12 text-muted-foreground" /> <CardTitle>No Questions</CardTitle> </CardHeader> <CardContent><p className="text-muted-foreground">No questions loaded for this test, or you've finished. If this is unexpected, please contact your teacher.</p></CardContent> <CardFooter><Button onClick={() => handleSubmitTest(false, "no_questions_or_finished")} variant="outline" className="w-full">Submit & End Test</Button></CardFooter> </Card> </div> );}
    ...
    <div className="flex flex-col h-screen bg-slate-100 dark:bg-slate-900">
      ...
      <div className="sticky top-[var(--top-header-height,57px)] z-40 bg-background shadow-sm p-3 border-b border-border">
         <div className="flex justify-between items-center max-w-full px-2 sm:px-4">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
                ...
                <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-primary h-7 w-7" onClick={() => setTestSessionState('instructions')}><Info className="h-4 w-4" /></Button>
            </div>
        </div>
      </div>
      <div className={cn("flex-1 flex max-w-full p-2 sm:p-4 gap-2 sm:gap-4 overflow-hidden")}>
        <Card className="flex-1 flex flex-col bg-card shadow-xl rounded-lg border border-border overflow-hidden">
            <CardHeader className="p-3 sm:p-4 border-b border-border bg-muted/30"><div className="flex justify-between items-center"><p className="text-xs sm:text-sm font-medium text-muted-foreground">Question {currentQuestionIndex + 1} of {questions.length}</p><div className="flex items-center gap-1">{currentQuestion.marks && <Badge variant="outline" className="text-xs px-1.5 py-0.5">Marks: {currentQuestion.marks}</Badge>}</div></div></CardHeader>
