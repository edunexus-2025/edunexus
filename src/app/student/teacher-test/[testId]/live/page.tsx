...
import type { StudentBookmark, User, TeacherTestAttempt } from '@/lib/types'; // Use TeacherTestAttempt
...
const getPbFileUrl = (record: RecordModel | null | undefined, fieldName: string): string | null => {
  if (record && record[fieldName] && typeof record[fieldName] === 'string') {
    if (isValidHttpUrl(record[fieldName] as string)) {
...
export default function StudentTeacherTestLivePage() {
  ...
  const fetchTestDataAndDecideStage = useCallback(async (isMountedGetter: () => boolean) => {
    ...
    try {
      ...
      if (fetchedTest.status !== 'Published') { if(isMountedGetter()) setError("This test is not currently published or available."); if(isMountedGetter()) setIsLoadingPage(false); return; }
      ...
      if (fetchedTest.Admin_Password && sessionStorage.getItem(pinSessionKey) !== 'true') {
        if(isMountedGetter()) setTestSessionState('pinEntry');
      } else {
        if(isMountedGetter()) setTestSessionState('instructions');
      }
    } catch (err: any) { if (isMountedGetter()) { setError(`Could not load test details. Error: ${err.data?.message || err.message}`); console.error("Error fetching test details for stage decision:", err); } }
  ...
  const loadQuestions = useCallback(async () => {
    if (!testDetails?.questions || testDetails.questions.length === 0 || !testDetails.teacherId) { setError("No questions associated with this test or teacher ID missing."); setIsLoadingPage(false); return; }
    setIsLoadingPage(true);
    try {
      const questionIds = testDetails.questions;
      const questionRecordsPromises = questionIds.map(id => pb.collection('teacher_question_data').getOne<QuestionRecord>(id, { '$autoCancel': false }).catch(err => { console.warn(`Failed to fetch question ${id} from 'teacher_question_data':`, err.data || err); return null; }));
      ...
      if (fetchedQuestionsFromDb.length === 0) { setError("No questions found for this test setup. The teacher may need to add questions."); }
    } catch (err: any) { setError(`Could not load questions. Error: ${err.data?.message || err.message}`); }
    finally { setIsLoadingPage(false); }
  }, [testDetails]);

  const handleStartTestAfterInstructions = () => { if (!testDetails) { toast({ title: "Error", description: "Test details not loaded.", variant: "destructive" }); return; } const durationMinutes = parseInt(testDetails.duration || "0", 10); setTimeLeft(durationMinutes > 0 ? durationMinutes * 60 : 3600); setTestSessionState('inProgress'); loadQuestions(); };
    ...
    return (
      <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/80 p-4">
        <Card className="w-full max-w-sm shadow-xl bg-card text-foreground">
          <CardHeader><CardTitle className="text-xl flex items-center gap-2"><KeyRound className="text-primary"/>Enter Test PIN</CardTitle><CardDescription>This test, "<span className="font-semibold">{testDetails?.testName || "Test"}</span>", requires a PIN provided by {teacherName}.</CardDescription></CardHeader>
          ...
          <CardFooter className="flex-col gap-2">
            <Button onClick={handlePinSubmit} className="w-full" disabled={isVerifyingPin || !enteredPin}> {isVerifyingPin && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>} Verify PIN & Continue </Button>
  ...
  if (!currentQuestion && !isLoadingPage && testSessionState === 'inProgress') { return ( <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/90 p-4 text-white"> <Card className="w-full max-w-lg text-center shadow-xl bg-background text-foreground"> <CardHeader> <AlertCircle className="mx-auto h-12 w-12 text-muted-foreground" /> <CardTitle>No Questions</CardTitle> </CardHeader> <CardContent><p className="text-muted-foreground">No questions available for this test.</p></CardContent> <CardFooter><Button onClick={() => router.back()} variant="outline" className="w-full">Go Back</Button></CardFooter> </Card> </div> );}
