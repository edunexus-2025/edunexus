...
interface QuestionRecord extends RecordModel {
  ...
  // Fields to store processed URLs for display
...
const getPbFileUrl = (record: RecordModel | null | undefined, fieldName: string): string | null => {
  if (record && record[fieldName] && record.collectionId && record.collectionName && typeof record[fieldName] === 'string') {
    ...
    catch (e) { console.warn(`StudentTeacherTestLivePage: Error getting URL for ${fieldName} in record ${record.id}:`, e); return null; }
  }
  return null;
};
...
export default function StudentTeacherTestLivePage() {
  ...
  const [isLoadingPage, setIsLoadingPage] = useState(true); // Correctly initialized
  ...
  const fetchTestDataAndDecideStage = useCallback(async (isMountedGetter: () => boolean) => {
    ...
    if (!currentTestId || !user?.id) { 
        if (isMountedGetter()) { 
            setError(currentTestId ? "User not authenticated." : "Invalid test ID."); 
    ...
    try {
      console.log(`[LivePage] Fetching test details for ID: ${currentTestId}`);
      ...
      console.log(`[LivePage] Fetched test details:`, fetchedTest);
      ...
      if (fetchedTest.Admin_Password && String(fetchedTest.Admin_Password).trim() !== "" && !pinIsVerifiedInSession) {
        console.log(`[LivePage] PIN required and not verified for test ${currentTestId}. State: pinEntry`);
        if(isMountedGetter()) setTestSessionState('pinEntry');
      } else {
        console.log(`[LivePage] No PIN required or PIN already verified for test ${currentTestId}. State: instructions`);
      ...
      if (isMountedGetter()) { 
        ...
        console.error("[LivePage] Error fetching test details for stage decision:", err); 
      }
    }
  ...
  useEffect(() => { 
    let isMounted = true; 
    if (!isAuthLoading) fetchTestDataAndDecideStage(() => isMounted); 
    return () => { isMounted = false; }; 
  }, [testId, isAuthLoading, fetchTestDataAndDecideStage]);

  const handlePinSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!testDetails || testDetails.Admin_Password === undefined) { setPinError("Test details or PIN not loaded."); return; }
  ...
  const loadQuestions = useCallback(async () => {
    if (!testDetails) { setError("Test details are missing, cannot load questions."); setIsLoadingPage(false); return; }
    if (!testDetails.questions || testDetails.questions.length === 0) { setError("No questions are linked to this test by the teacher."); setIsLoadingPage(false); setQuestions([]); return; }
    if (!testDetails.teacherId) { setError("Teacher ID is missing from test details, cannot fetch questions reliably."); setIsLoadingPage(false); setQuestions([]); return; }

    setIsLoadingPage(true); setError(null); 
    try {
      const questionIds = testDetails.questions;
      console.log("[LivePage] loadQuestions. IDs from teacher_tests:", questionIds);
      if (questionIds.length === 0) { setError("No questions are linked to this test by the teacher."); setQuestions([]); setIsLoadingPage(false); return; }
      const questionRecordsPromises = questionIds.map(id => pb.collection('teacher_question_data').getOne<QuestionRecord>(id, { '$autoCancel': false }).catch(err => { console.error(`[LivePage] Failed to fetch question ${id} from 'teacher_question_data':`, err.data || err); return null; }));
      const resolvedQuestions = await Promise.all(questionRecordsPromises);
      const fetchedQuestionsFromDb = resolvedQuestions.filter(q => q !== null).map(q => ({ ...(q as QuestionRecord), displayQuestionImageUrl: getPbFileUrl(q, 'QuestionImage'), displayOptionAImageUrl: getPbFileUrl(q, 'OptionAImage'), displayOptionBImageUrl: getPbFileUrl(q, 'OptionBImage'), displayOptionCImageUrl: getPbFileUrl(q, 'OptionCImage'), displayOptionDImageUrl: getPbFileUrl(q, 'OptionDImage'), displayExplanationImageUrl: getPbFileUrl(q, 'explanationImage'), marks: typeof q?.marks === 'number' ? q.marks : 1, }));
      console.log("[LivePage] Successfully fetched and mapped questions:", fetchedQuestionsFromDb.length);
      setQuestions(fetchedQuestionsFromDb);
      const initialAnswers: Record<string, AnswerLogItem> = {}; fetchedQuestionsFromDb.forEach(q => { initialAnswers[q.id] = { questionId: q.id, selectedOption: null, isCorrect: null, markedForReview: false, timeSpentSeconds: 0 }; });
      setUserAnswers(initialAnswers); questionStartTimeRef.current = Date.now();
      if (fetchedQuestionsFromDb.length === 0) { setError("No questions found for this test setup. The teacher may need to add questions, or API rules for 'teacher_question_data' might be too restrictive (needs to be @request.auth.id != '')."); }
  ...
  const handleSubmitTest = useCallback(async (autoSubmit = false, terminationReason?: string) => {
    if (!user || !testDetails || !testDetails.expand?.teacherId || isSubmittingTest || testSessionState === 'completed' || testSessionState === 'terminated') { console.warn("handleSubmitTest (teacher test) blocked. Conditions not met."); return; }
    setIsSubmittingTest(true); if(currentQuestion && userAnswers[currentQuestion.id] && questionStartTimeRef.current && testSessionState === 'inProgress') { const currentTime = Date.now(); const timeSpentCurrentQuestion = Math.round((currentTime - questionStartTimeRef.current) / 1000); userAnswers[currentQuestion.id].timeSpentSeconds = (userAnswers[currentQuestion.id].timeSpentSeconds || 0) + timeSpentCurrentQuestion; }
    let correctCount = 0; let attemptedCount = 0; let pointsEarnedFromTest = 0;
    const answersLogForDb = questions.map(q => { const userAnswerRec = userAnswers[q.id]; const selected = userAnswerRec?.selectedOption || null; let isCorrectAns = false; const correctOptionValue = q.CorrectOption?.replace("Option ", "") as "A" | "B" | "C" | "D" | undefined; if (selected) { attemptedCount++; if (selected === `Option ${correctOptionValue}`) { correctCount++; isCorrectAns = true; pointsEarnedFromTest += (q.marks || 1); } } return { questionId: q.id, selectedOption: selected, correctOption: correctOptionValue ? `Option ${correctOptionValue}` : null, isCorrect: isCorrectAns, markedForReview: userAnswerRec?.markedForReview || false, timeSpentSeconds: userAnswerRec?.timeSpentSeconds || 0 }; });
    const maxScorePossible = questions.reduce((sum, q) => sum + (q.marks || 1), 0); const percentage = maxScorePossible > 0 ? (pointsEarnedFromTest / maxScorePossible) * 100 : 0; const finalTestStatusString: TeacherTestAttempt['status'] = terminationReason === 'time_up' ? 'terminated_time_up' : 'completed'; const durationTaken = testDetails?.duration ? parseInt(testDetails.duration, 10) * 60 - (timeLeft || 0) : 0;
    const resultData = { student: user.id, teacher_test: testDetails.id, teacher: testDetails.expand.teacherId.id, test_name_cache: testDetails.testName, score: pointsEarnedFromTest, max_score: maxScorePossible, percentage: parseFloat(percentage.toFixed(2)), answers_log: JSON.stringify(answersLogForDb), duration_taken_seconds: durationTaken, status: finalTestStatusString, attempt_date: new Date().toISOString(), start_time: new Date(Date.now() - durationTaken * 1000).toISOString(), end_time: new Date().toISOString(), total_questions_in_test: questions.length, attempted_questions: attemptedCount, correct_answers: correctCount, incorrect_answers: attemptedCount - correctCount, unattempted_questions: questions.length - attemptedCount, plan_type_at_attempt: "TeacherPlan_Student", }; 
    try { const createdResultRecord = await pb.collection('teacher_test_attempts').create(resultData); sessionStorage.removeItem(`${TEST_PIN_SESSION_KEY_PREFIX}${testId}`); setTestSessionState(finalTestStatusString === 'completed' ? 'completed' : 'terminated'); setTimeLeft(0); toast({ title: autoSubmit ? (terminationReason ? "Test Terminated" : "Test Auto-Submitted") : "Test Submitted Successfully!", description: `Your results for "${testDetails.testName}" have been recorded.` }); router.push(Routes.studentTeacherTestResult(createdResultRecord.id)); } catch (err: any) { console.error("Failed to submit teacher test results:", err); toast({ title: "Submission Failed", description: `Could not save results: ${err.data?.message || err.message}`, variant: "destructive" }); } finally { setIsSubmittingTest(false); }
  }, [user, testDetails, questions, userAnswers, timeLeft, router, toast, isSubmittingTest, testSessionState, currentQuestion, testId, authRefresh]);

  useEffect(() => { if (timeLeft === 0 && testSessionState === 'inProgress') { handleSubmitTest(true, "time_up"); } }, [timeLeft, testSessionState, handleSubmitTest]);
  useEffect(() => { if (timerIntervalRef.current) clearInterval(timerIntervalRef.current); if (testSessionState === 'inProgress' && timeLeft !== null && timeLeft > 0) { timerIntervalRef.current = setInterval(() => setTimeLeft(prev => (prev ? prev - 1 : 0)), 1000); } return () => { if (timerIntervalRef.current) clearInterval(timerIntervalRef.current); }; }, [testSessionState, timeLeft]);
  ...
  if (isAuthLoading || (isLoadingPage && testSessionState === 'pinEntry')) { return ( <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/90 p-4 text-white"> <Loader2 className="h-12 w-12 animate-spin text-primary mb-4" /> <p className="text-lg">Loading test environment...</p> </div> ); }
  if (error && testSessionState !== 'pinEntry') { return ( <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/90 p-4 text-white"> <Card className="w-full max-w-lg text-center shadow-xl bg-background text-foreground"> <CardHeader> <AlertCircle className="mx-auto h-12 w-12 text-destructive" /> <CardTitle className="text-destructive">Error Loading Test</CardTitle> </CardHeader> <CardContent><p className="text-muted-foreground whitespace-pre-wrap">{error}</p></CardContent> <CardFooter><Button onClick={() => router.back()} variant="outline" className="w-full">Go Back</Button></CardFooter> </Card> </div> ); }
    ...
    return (
      <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/80 p-4">
        <Card className="w-full max-w-sm shadow-xl bg-card text-foreground">
          <CardHeader><CardTitle className="text-xl flex items-center gap-2"><KeyRound className="text-primary"/>Enter Test PIN</CardTitle><CardDescription>This test requires a PIN provided by {teacherName}. Test: {testDetails?.testName || 'Loading...'}</CardDescription></CardHeader>
          <CardContent className="space-y-4">
            {isLoadingPage ? <Skeleton className="h-10 w-full" /> :
            <form onSubmit={handlePinSubmit} className="space-y-4">
              ...
              <Button type="submit" className="w-full" disabled={isVerifyingPin || !enteredPin || isLoadingPage}> {isVerifyingPin && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>} Verify PIN & Continue </Button>
            </form>}
          </CardContent>
           <CardFooter><Button variant="link" size="sm" onClick={() => router.back()} className="text-xs text-muted-foreground mx-auto">Cancel & Go Back</Button></CardFooter>
        </Card>
      </div>
    );
  }
    ...
    return (
      <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/80 p-4">
        <Card className="w-full max-w-2xl shadow-xl bg-card text-foreground">
          <CardHeader><CardTitle className="text-2xl">Test Instructions: {testDetails?.testName}</CardTitle><CardDescription>Read carefully before starting. Duration: {testDetails?.duration || 'N/A'} minutes. Questions: {testDetails?.questions?.length || 'N/A'}</CardDescription></CardHeader>
          <CardContent className="max-h-[60vh] overflow-y-auto prose prose-sm dark:prose-invert"><p>This test is conducted by: {teacherName}. All the best!</p></CardContent>
          <CardFooter className="justify-center"><Button onClick={handleStartTestAfterInstructions} size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground">I'm Ready, Start Test!</Button></CardFooter>
        </Card>
      </div>
    );
  }
  ...
  if (!currentQuestion && !isLoadingPage && testSessionState === 'inProgress') { return ( <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/90 p-4 text-white"> <Card className="w-full max-w-lg text-center shadow-xl bg-background text-foreground"> <CardHeader> <AlertCircle className="mx-auto h-12 w-12 text-muted-foreground" /> <CardTitle>No Questions</CardTitle> </CardHeader> <CardContent><p className="text-muted-foreground">{error || "No questions available for this test, or an error occurred loading them."}</p></CardContent> <CardFooter><Button onClick={() => router.back()} variant="outline" className="w-full">Go Back</Button></CardFooter> </Card> </div> );}
  
  return (
    <div className="flex flex-col h-screen bg-slate-100 dark:bg-slate-900">
      <header className="sticky top-0 z-50 bg-card shadow-md p-3 border-b border-border"><div className="flex justify-between items-center max-w-full px-2 sm:px-4"><div className="text-xs sm:text-sm font-medium text-muted-foreground truncate max-w-[calc(33%-20px)] sm:max-w-xs" title={testDetails?.testName || 'Test'}>{testDetails?.testName || 'Test Name'} <span className="hidden sm:inline">- By {teacherName}</span></div><div className="flex-shrink-0"> <AppLogo mainTextSize="text-lg" taglineTextSize="text-[0px]" iconSize={22} /> </div><div className="text-sm font-medium text-muted-foreground bg-muted/50 px-2 py-1 rounded-md shadow-sm flex items-center w-auto max-w-[calc(33%-20px)] sm:max-w-xs justify-end"><Clock className="h-4 w-4 mr-1.5 flex-shrink-0" /> <span className="truncate">{formatTime(timeLeft)}</span></div></div></header>
      <div className="sticky top-[var(--top-header-height,57px)] z-40 bg-background shadow-sm p-3 border-b border-border"><div className="flex justify-between items-center max-w-full px-2 sm:px-4"><div className="text-sm font-semibold text-foreground truncate max-w-[calc(50%-120px)] sm:max-w-md" title={currentQuestion?.subject || testDetails?.QBExam || 'Subject'}>SUBJECT: {currentQuestion?.subject || testDetails?.QBExam || 'N/A'}</div><div className="flex items-center gap-1 sm:gap-2"><Sheet open={isMobileSheetOpen} onOpenChange={setIsMobileSheetOpen}><SheetTrigger asChild><Button variant="ghost" size="icon" className="text-muted-foreground hover:text-primary h-7 w-7 md:hidden" aria-label="Open Question Navigation"><ListOrdered className="h-5 w-5" /></Button></SheetTrigger><SheetContent side="right" className="w-3/4 p-0 flex flex-col"><ShadcnSheetHeader className="p-3 border-b text-center"><ShadcnSheetTitle className="text-lg">Navigation</ShadcnSheetTitle><ShadcnSheetDescription>Jump to any question or submit.</ShadcnSheetDescription></ShadcnSheetHeader><QuestionPaletteContent /></SheetContent></Sheet><Button variant="ghost" size="icon" className="text-muted-foreground hover:text-primary h-7 w-7 hidden md:inline-flex" onClick={() => setIsRightSidebarOpen(!isRightSidebarOpen)} aria-label={isRightSidebarOpen ? "Hide Question Panel" : "Show Question Panel"}><MoreVertical className="h-5 w-5" /></Button><Button variant="ghost" size="icon" className="text-muted-foreground hover:text-primary h-7 w-7" onClick={() => setTestSessionState('instructions')} title="View Instructions"><Info className="h-4 w-4" /></Button></div></div></div>
