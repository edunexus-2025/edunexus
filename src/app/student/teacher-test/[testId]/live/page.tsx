...
// Interface for the main test details fetched from 'teacher_tests'
...
// Unified interface for questions from either source after mapping
interface QuestionRecord {
  ...
  marks?: number;
  subject?: string; // Can be different per question for full length tests
  source: 'edunexus' | 'teacher'; // To know the origin
}

// Intermediate type for records fetched from PocketBase before mapping to QuestionRecord
interface FetchedQuestionRecord extends RecordModel {
  ...
  explanationText?: string; // For question_bank
  explanationImage?: string | null; // For question_bank
  difficulty?: 'Easy' | 'Medium' | 'Hard';
  marks?: number;
  subject?: string;

  // Fields from teacher_question_data (prefix 'Question' or 'Option' or 'Correct')
  QuestionText?: string;
  ...
  explanationTextTeacher?: string; // Assuming teacher_question_data uses 'explanationText'
  explanationImageTeacher?: string | null; // Assuming teacher_question_data uses 'explanationImage' (URL)
...
const getPbFileUrl = (record: RecordModel | null | undefined, fieldName: string): string | null => {
  if (record && record[fieldName] && record.collectionId && record.collectionName && typeof record[fieldName] === 'string') {
    ...
    return record[fieldName] as string; // Already a URL
  }
  return null;
};
...
export default function StudentTeacherTestLivePage() {
  ...
  const fetchTestDataAndDecideStage = useCallback(async (isMountedGetter: () => boolean) => {
    ...
    try {
      console.log(`[FETCH_TEST_DATA_DEBUG] Fetching 'teacher_tests' record for ID: ${testId}. Fetching fields: id,testName,status,Admin_Password,duration,teacherId,model,QBExam,questions_edunexus,questions_teachers and expanding teacherId,questions_edunexus,questions_teachers`);
      const fetchedTest = await pb.collection('teacher_tests').getOne<TeacherTestDetailsRecord>(testId, {
        fields: 'id,testName,status,Admin_Password,duration,teacherId,model,QBExam,questions_edunexus,questions_teachers,expand.teacherId.name,expand.questions_edunexus.id,expand.questions_teachers.id', // Minimize expanded fields initially
        expand: 'teacherId,questions_edunexus,questions_teachers',
        '$autoCancel': false,
      });
      console.log("[FETCH_TEST_DATA_DEBUG] Fetched teacher_tests record:", JSON.parse(JSON.stringify(fetchedTest)));
      ...
      setTestDetails(fetchedTest); // Store the full fetched test details
      setTeacherName(fetchedTest.expand?.teacherId?.name || 'Your Teacher');
      
      const eduNexusIds = fetchedTest.questions_edunexus || [];
      const teacherIds = fetchedTest.questions_teachers || [];
      const totalQuestionIds = eduNexusIds.length + teacherIds.length;
      
      setInstructionScreenQuestionCount(totalQuestionIds);
      console.log(`[FETCH_TEST_DATA_DEBUG] Instruction screen question count set to: ${totalQuestionIds}. EduNexus IDs: ${eduNexusIds.length}, Teacher IDs: ${teacherIds.length}`);
      ...
      if (pinRequired && !pinIsVerifiedInSession) {
        console.log("[FETCH_TEST_DATA_DEBUG] PIN required and not verified in session. Setting state to 'pinEntry'.");
        setTestSessionState('pinEntry');
        setIsLoadingPageData(false); 
        return; 
      }
      console.log("[FETCH_TEST_DATA_DEBUG] No PIN required or PIN already verified. Setting state to 'instructions'.");
      ...
      if (isMountedGetter()) {
        ...
        console.error("[FETCH_TEST_DATA_DEBUG] Error fetching teacher_tests:", errorMsg, "Full error:", clientError);
        setError(errorMsg);
  ...
  const loadQuestions = useCallback(async (isMountedGetter: () => boolean) => {
    if (!testDetails) { 
      if (isMountedGetter()) { 
        setError("Test details not available to load questions."); 
        setIsLoadingPageData(false); 
        setQuestions([]); 
      } 
      return; 
    }
    console.log("[LOAD_QUESTIONS_DEBUG] Inside loadQuestions. testDetails available.");
    console.log("[LOAD_QUESTIONS_DEBUG] testDetails.questions_edunexus (IDs):", JSON.stringify(testDetails.questions_edunexus));
    console.log("[LOAD_QUESTIONS_DEBUG] testDetails.questions_teachers (IDs):", JSON.stringify(testDetails.questions_teachers));
    console.log("[LOAD_QUESTIONS_DEBUG] testDetails.expand?.questions_edunexus (Expanded Data):", testDetails.expand?.questions_edunexus ? `${testDetails.expand.questions_edunexus.length} records` : 'undefined');
    console.log("[LOAD_QUESTIONS_DEBUG] testDetails.expand?.questions_teachers (Expanded Data):", testDetails.expand?.questions_teachers ? `${testDetails.expand.questions_teachers.length} records` : 'undefined');

    const eduNexusQuestionIds = testDetails.questions_edunexus || [];
    const teacherQuestionIds = testDetails.questions_teachers || [];
    ...
    try {
      // Process expanded EduNexus questions if available
      if (testDetails.expand?.questions_edunexus && Array.isArray(testDetails.expand.questions_edunexus)) {
        testDetails.expand.questions_edunexus.forEach((q: FetchedQuestionRecord) => {
          combinedQuestions.push({
            ...
            marks: q.marks, subject: q.subject, source: 'edunexus'
          });
        });
      } else if (eduNexusQuestionIds.length > 0) {
        const eduNexusFilter = eduNexusQuestionIds.map(id => `id = "${escapeForPbFilter(id)}"`).join(' || ');
        console.log("[LOAD_QUESTIONS_DEBUG] Fetching EduNexus questions from 'question_bank' with filter:", eduNexusFilter);
      ...
      // Process expanded Teacher questions if available
      if (testDetails.expand?.questions_teachers && Array.isArray(testDetails.expand.questions_teachers)) {
        testDetails.expand.questions_teachers.forEach((q: FetchedQuestionRecord) => {
          combinedQuestions.push({
            id: q.id, displayQuestionText: q.QuestionText, displayQuestionImageUrl: q.QuestionImage, // Directly use URL
            ...
            displayCorrectOptionLabel: q.CorrectOption?.replace("Option ", "") || "", displayExplanationText: q.explanationText, displayExplanationImageUrl: q.explanationImage, // Direct URL
            marks: q.marks, subject: q.subject, source: 'teacher'
          });
        });
      } else if (teacherQuestionIds.length > 0) {
        const teacherFilter = teacherQuestionIds.map(id => `id = "${escapeForPbFilter(id)}"`).join(' || ');
        console.log("[LOAD_QUESTIONS_DEBUG] Fetching Teacher questions from 'teacher_question_data' with filter:", teacherFilter);
        ...
        teacherRecords.forEach(q => {
           combinedQuestions.push({
            id: q.id, displayQuestionText: q.QuestionText, displayQuestionImageUrl: q.QuestionImage, // Direct URL
            displayOptions: [ { label: 'A', text: q.OptionAText, imageUrl: q.OptionAImage }, { label: 'B', text: q.OptionBText, imageUrl: q.OptionBImage }, { label: 'C', text: q.OptionCText, imageUrl: q.OptionCImage }, { label: 'D', text: q.OptionDText, imageUrl: q.OptionDImage } ],
            displayCorrectOptionLabel: q.CorrectOption?.replace("Option ", "") || "", displayExplanationText: q.explanationText, displayExplanationImageUrl: q.explanationImage, // Direct URL
            marks: q.marks, subject: q.subject, source: 'teacher'
          });
      ...
      if (combinedQuestions.length === 0 && (eduNexusQuestionIds.length > 0 || teacherQuestionIds.length > 0)) {
        if (isMountedGetter()) {
          setError("No questions were loaded. This test might be empty or there's an issue with question linking or permissions. Teacher may need to add questions via their panel.");
          setQuestions([]);
        }
      } else {
      ...
      if (isMountedGetter()) {
        const clientError = err as PocketBaseClientResponseError;
        console.error("[LOAD_QUESTIONS_DEBUG] Error loading questions:", clientError.data || clientError);
        setError(`Critical error loading question content: ${clientError.data?.message || clientError.message}. Ensure collection API rules allow access.`);
      }
    } finally {
      if (isMountedGetter()) setIsLoadingPageData(false);
    }
  ...
  if (!currentQuestion && !isLoadingPageData && testSessionState === 'in_progress') { return ( <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/90 p-4 text-white"> <Card className="w-full max-w-lg text-center shadow-xl bg-background text-foreground"> <CardHeader> <AlertCircle className="mx-auto h-12 w-12 text-muted-foreground" /> <CardTitle>No Questions</CardTitle> </CardHeader> <CardContent><p className="text-muted-foreground whitespace-pre-wrap">No questions loaded for this test, or you've finished. If this is unexpected, please contact your teacher. Error: {error}</p></CardContent> <CardFooter><Button onClick={() => handleSubmitTest(false, 'no_questions_or_finished')} variant="outline" className="w-full">Submit & End Test</Button></CardFooter> </Card> </div> );}
    ...
    <div className="flex flex-col h-screen bg-slate-100 dark:bg-slate-900">
      <header className="sticky top-0 z-50 bg-card shadow-md p-3 border-b border-border">
        <div className="flex justify-between items-center max-w-full px-2 sm:px-4">
          <div className="text-xs sm:text-sm font-medium text-muted-foreground truncate max-w-[calc(33%-20px)] sm:max-w-xs" title={testDetails?.testName || 'Test'}>{testDetails?.testName || 'Test Name'} <span className="hidden sm:inline">- By {teacherName}</span></div>
