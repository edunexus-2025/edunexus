...
export default function StudentTeacherTestLivePage() {
  ...
  const [isLoadingPage, setIsLoadingPage] = useState(isLoadingPage);
  ...
  const loadQuestions = useCallback(async () => {
    if (!testDetails) {
      setError("Test details are missing, cannot load questions.");
      setIsLoadingPage(false);
      return;
    }
    ...
    if (!testDetails.teacherId) {
        setError("Teacher ID is missing from test details, cannot fetch questions reliably.");
        setIsLoadingPage(false);
        setQuestions([]);
        return;
    }
    ...
    try {
      ...
      console.log("[LivePage] Loading questions. IDs from teacher_tests:", questionIds);
      ...
      const questionRecordsPromises = questionIds.map(id => 
        pb.collection('teacher_question_data').getOne<QuestionRecord>(id, { '$autoCancel': false })
          .catch(err => { console.error(`[LivePage] Failed to fetch question ${id} from 'teacher_question_data':`, err.data || err); return null; })
      );
      ...
      const fetchedQuestionsFromDb = resolvedQuestions.filter(q => q !== null).map(q => ({ 
        ...(q as QuestionRecord), 
        displayQuestionImageUrl: getPbFileUrl(q, 'QuestionImage'), // Uses QuestionImage from teacher_question_data
      ...
      const initialAnswers: Record<string, AnswerLogItem> = {}; 
      ...
      if (fetchedQuestionsFromDb.length === 0) {
         setError("No questions found for this test setup. The teacher may need to add questions, or API rules for 'teacher_question_data' might be too restrictive (needs to be @request.auth.id != '').");
      }
  ...
  const handleSubmitTest = useCallback(async (autoSubmit = false, terminationReason?: string) => {
    if (!user || !testDetails || !testDetails.expand?.teacherId || isSubmittingTest || testSessionState === 'completed' || testSessionState === 'terminated') return;
    ...
    const resultData = { student: user.id, teacher_test: testDetails.id, teacher: testDetails.expand.teacherId.id, test_name_cache: testDetails.testName, score_obtained: pointsEarnedFromTest, max_score_possible: maxScorePossible, percentage: parseFloat(percentage.toFixed(2)), answers_log: JSON.stringify(answersLogForDb), duration_taken_seconds: durationTaken, status: finalTestStatusString, attempt_date: new Date().toISOString(), start_time: new Date(Date.now() - durationTaken * 1000).toISOString(), end_time: new Date().toISOString(), total_questions_in_test: questions.length, attempted_questions: attemptedCount, correct_answers: correctCount, incorrect_answers: attemptedCount - correctCount, unattempted_questions: questions.length - attemptedCount, plan_type_at_attempt: "TeacherPlan_Student", }; 
    try { const createdResultRecord = await pb.collection('teacher_test_attempts').create(resultData); sessionStorage.removeItem(`${TEST_PIN_SESSION_KEY_PREFIX}${testId}`); setTestSessionState(finalTestStatusString === 'completed' ? 'completed' : 'terminated'); setTimeLeft(0); toast({ title: autoSubmit ? (terminationReason ? "Test Terminated" : "Test Auto-Submitted") : "Test Submitted Successfully!", description: `Your results for "${testDetails.testName}" have been recorded.` }); router.push(Routes.testResultTeacherTest(createdResultRecord.id)); } catch (err: any) { console.error("Failed to submit teacher test results:", err); toast({ title: "Submission Failed", description: `Could not save results: ${err.data?.message || err.message}`, variant: "destructive" }); } finally { setIsSubmittingTest(false); }
  }, [user, testDetails, questions, userAnswers, timeLeft, router, toast, isSubmittingTest, testSessionState, currentQuestion, testId]);
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center p-4 bg-gradient-to-br from-primary/10 via-background to-background">
        <Card className="w-full max-w-md shadow-xl">
          <CardHeader><CardTitle className="text-2xl flex items-center gap-2"><KeyRound className="text-primary"/>Enter Test PIN</CardTitle><CardDescription>This test, "<span className="font-semibold">{testDetails?.testName || "Test"}</span>", requires a PIN provided by {teacherName}.</CardDescription></CardHeader>
          <CardContent>
            <form onSubmit={handlePinSubmit} className="space-y-4">
              ...
              <Button type="submit" className="w-full" disabled={isVerifyingPin || !enteredPin}> {isVerifyingPin && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>} Verify PIN & Continue </Button>
            </form>
          </CardContent>
        </Card>
      </div>
  ...
  if (testSessionState === 'instructions') {
    return ( <div className="flex flex-col min-h-screen bg-slate-100 dark:bg-slate-950 p-4 md:p-8 items-center justify-center"> <Card className="w-full max-w-4xl shadow-2xl rounded-lg bg-card text-foreground"> <CardHeader className="text-center border-b pb-4"> <CardTitle className="text-3xl font-bold text-foreground">Instructions: {testDetails?.testName}</CardTitle> <CardDescription className="text-md text-muted-foreground mt-1">Please read carefully before starting.</CardDescription> </CardHeader> <CardContent className="p-0"> <ScrollArea className="h-[calc(80vh-180px)] md:h-[calc(70vh-160px)]"> <div className="p-6 prose prose-sm sm:prose-base dark:prose-invert max-w-none text-foreground"><p>This test contains {questions.length || testDetails?.questions?.length || 'multiple'} questions. Duration: {testDetails?.duration || 'N/A'} minutes. All the best!</p></div></ScrollArea></CardContent><CardFooter className="p-6 border-t justify-center"><Button onClick={handleStartTestAfterInstructions} size="lg" className="px-10 py-6 text-base bg-primary hover:bg-primary/90 text-primary-foreground shadow-md hover:shadow-lg transition-all">I am ready to Begin</Button></CardFooter></Card></div> );
  }
