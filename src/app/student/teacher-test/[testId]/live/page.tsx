...
interface FetchedQuestionSourceRecord extends RecordModel {
  id: string;
  // Fields from question_bank (lowercase)
  questionText?: string | null;
  optionAText?: string | null; optionBText?: string | null; optionCText?: string | null; optionDText?: string | null;
  questionImage?: string | null; // Filename
  optionAImage?: string | null; optionBImage?: string | null; optionCImage?: string | null; optionDImage?: string | null;
  ...
  OptionAImage_teacher?: string | null; OptionBImage_teacher?: string | null; OptionCImage_teacher?: string | null; OptionDImage_teacher?: string | null;
...
const getPbFileUrl = (record: RecordModel, fieldName: string, filename?: string | null): string | null => {
  const file = filename || record[fieldName];
  if (record && file && record.collectionId && record.collectionName && typeof file === 'string') {
    try { return pb.files.getUrl(record, file); }
    catch (e) { console.warn(`StudentTeacherTestLivePage (getPbFileUrl): Error for field '${fieldName}' in record '${record.id}'. Value: '${file}'. Collection: ${record.collectionName}. Error:`, e); return null; }
  }
  return null;
};

const renderLatexDisplay = (text: string | undefined | null): React.ReactNode => {
  if (!text) return null;
...
export default function StudentTakeTeacherTestLivePage() {
  ...
  const normalizeFetchedQuestion = (q: FetchedQuestionSourceRecord, source: 'question_bank' | 'teacher_question_data'): NormalizedQuestionRecord => {
    ...
    if (source === 'question_bank') {
      ...
      normalized.lessonName = q.lessonName;
  ...
  }, [escapeForPbFilter, normalizeFetchedQuestion]);
  ...
  const fetchTestDataAndDecideStage = useCallback(async (isMountedGetter: () => boolean) => {
    ...
    try {
      ...
      if (isMountedGetter()) {
        ...
        let errorMsg = `Could not load test data. Error: ${err.data?.message || err.message}.`;
  ...
  const renderOption = (option: NormalizedQuestionRecord['displayOptions'][0]): React.ReactNode => {
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 text-sm">
          ...
          {option.text && <div className="prose prose-sm dark:prose-invert max-w-none mt-0.5">{renderLatexDisplay(option.text)}</div>}
    ...
    <div className="flex flex-col h-screen bg-slate-100 dark:bg-slate-900">
      ...
      <div className={cn("flex-1 flex max-w-full p-2 sm:p-4 gap-2 sm:gap-4 overflow-hidden")}>
        <Card className="flex-1 flex flex-col bg-card shadow-xl rounded-lg border border-border overflow-hidden">
            <CardHeader className="p-3 sm:p-4 border-b border-border bg-muted/30"><div className="flex justify-between items-center"><p className="text-xs sm:text-sm font-medium text-muted-foreground">Question {currentQuestionIndex + 1} of {questions.length}</p><div className="flex items-center gap-1">{currentQuestion?.difficulty && <Badge variant={currentQuestion.difficulty === 'Easy' ? 'secondary' : currentQuestion.difficulty === 'Medium' ? 'default' : 'destructive'} className="text-xs px-1.5 py-0.5">{currentQuestion.difficulty}</Badge>}{currentQuestion?.marks !== undefined && <Badge variant="outline" className="text-xs px-1.5 py-0.5">Marks: {currentQuestion.marks}</Badge>}</div></div></CardHeader>
