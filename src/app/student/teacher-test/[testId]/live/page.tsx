...
interface TeacherQuestionRecord extends RecordModel {
  ...
  subject?: string;
  marks?: number;
  LessonName?: string; // This is now a relation ID to teacher_tests
  expand?: {
    LessonName?: { // If you expand the LessonName relation
        id: string;
        testName: string; // Or whatever display field you have for teacher_tests
    }
  }
}
...
export default function StudentTeacherTestLivePage() {
  ...
  const [isLoadingPageData, setIsLoadingPageData] = useState(true); // Correct initialization
  ...
  const fetchTestDataAndDecideStage = useCallback(async (isMountedGetter: () => boolean) => {
    ...
    if (isMountedGetter()) { setIsLoadingPageData(true); setError(null); setTestDetails(null); setTeacherName('Educator'); setInstructionScreenQuestionCount(null); }

    try {
      const fetchedTest = await pb.collection('teacher_tests').getOne<TeacherTestDetailsRecord>(testId, {
        fields: 'id,testName,status,Admin_Password,duration,questions,teacherId,model,QBExam,expand.teacherId.id,expand.teacherId.name',
        expand: 'teacherId', 
        '$autoCancel': false,
      });
      ...
      if (isMountedGetter()) {
        ...
        setInstructionScreenQuestionCount(Array.isArray(fetchedTest.questions) ? fetchedTest.questions.length : 0);
  ...
  const loadQuestions = useCallback(async (isMountedGetter: () => boolean) => {
    if (!testDetails || !testDetails.questions || testDetails.questions.length === 0) {
      if (isMountedGetter()) { setError("No questions associated with this test. Please contact the teacher."); setQuestions([]); setIsLoadingPageData(false); }
      return;
    }
    if (!testDetails.teacherId) {
      if (isMountedGetter()) { setError("Teacher ID missing from test, cannot load questions."); setQuestions([]); setIsLoadingPageData(false); }
      return;
    }
    ...
    try {
      ...
      if (fetchedQuestionsFromDb.length === 0) {
        if (isMountedGetter()) setError("Could not load any questions for this test. Ensure questions are linked and API rules for 'teacher_question_data' allow student access.");
      }
      ...
      if (isMountedGetter()) {
        const clientError = err as PocketBaseClientResponseError;
        setError(`Critical error loading questions: ${clientError.data?.message || clientError.message}. Check API Rules for 'teacher_question_data' and question links.`);
      }
    } finally {
      if (isMountedGetter()) setIsLoadingPageData(false);
    }
  ...
  const handleSubmitTest = useCallback(async (autoSubmit = false, terminationReason?: string) => {
    ...
    const answersLog = questions.map(q => {
      ...
      const correctOptionValueFromRecord = q.CorrectOption; // Directly use "Option A", "Option B" etc.
      const questionMarks = typeof q.marks === 'number' ? q.marks : 1;
      
      if (selected) { 
        attemptedCount++; 
        if (selected === correctOptionValueFromRecord) { 
          correctCount++; 
          isCorrectAns = true; 
