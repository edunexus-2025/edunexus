...
interface TeacherTestRecord extends RecordModel {
  ...
  questions?: string[]; // Added to store question IDs
  expand?: {
...
export default function StudentTeacherTestLivePage() {
  ...
  const [testSessionState, setTestSessionState] = useState<'pin_entry' | 'instructions' | 'in_progress' | 'completed' | 'terminated'>('pin_entry');
  ...
  const fetchTestDataForTeacherTest = useCallback(async (isMountedGetter: () => boolean) => {
    ...
    try {
      console.log(`StudentTakeTeacherTestPage: Fetching test details for ID: ${currentTestId}`);
      ...
      console.log("StudentTakeTeacherTestPage: Fetched test details:", fetchedTest);
      if (!fetchedTest.Admin_Password) { if(isMountedGetter()) { setError("This test is not properly configured (missing PIN). Cannot proceed."); setIsLoadingPage(false); } return; }
      setTestDetails(fetchedTest);
      if (fetchedTest.expand?.teacherId?.name) setTeacherName(fetchedTest.expand.teacherId.name);
      setTestSessionState('pin_entry'); // Start with PIN entry
  ...
  const loadQuestions = useCallback(async () => {
    ...
    try {
      ...
      console.log("StudentTakeTeacherTestPage: Question IDs from test details:", questionIds);
      ...
      const fetchedQuestionsFromDb = await pb.collection('teacher_question_data').getFullList<QuestionRecord>({
        filter: questionFilter, sort: '+created', '$autoCancel': false,
      });
      console.log("StudentTakeTeacherTestPage: Fetched questions from DB:", fetchedQuestionsFromDb.length, fetchedQuestionsFromDb);

      if (fetchedQuestionsFromDb.length === 0) { setError("Questions configured for this test could not be loaded or do not exist."); setQuestions([]); setIsLoadingPage(false); return; }

      const questionsWithUrls = fetchedQuestionsFromDb.map(q => ({ ...q, displayQuestionImageUrl: getPbFileUrl(q, 'QuestionImage'), displayOptionAImageUrl: getPbFileUrl(q, 'OptionAImage'), displayOptionBImageUrl: getPbFileUrl(q, 'OptionBImage'), displayOptionCImageUrl: getPbFileUrl(q, 'OptionCImage'), displayOptionDImageUrl: getPbFileUrl(q, 'OptionDImage'), displayExplanationImageUrl: getPbFileUrl(q, 'explanationImage'), marks: typeof q.marks === 'number' ? q.marks : 1, }));
  ...
  const handleStartTestAfterInstructions = () => {
    ...
    setTimeLeft(durationMinutes > 0 ? durationMinutes * 60 : 3600); // Default 1hr
    setTestSessionState('in_progress');
    loadQuestions(); // Load questions when test starts
  };
  ...
  const handleSubmitTest = useCallback(async (autoSubmit = false, terminationReason?: string) => {
    ...
    if(currentQuestion && userAnswers[currentQuestion.id] && questionStartTimeRef.current && testSessionState === 'in_progress') {
    ...
    const resultData = {
      ...
      plan_type_at_attempt: user.studentSubscriptionTier || "TeacherPlan_Student",
    };
    try {
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center p-4 bg-gradient-to-br from-primary/10 via-background to-background">
        <Card className="w-full max-w-md shadow-xl">
          <CardHeader><CardTitle className="text-2xl flex items-center gap-2"><KeyRound className="text-primary"/>Enter Test PIN</CardTitle><CardDescription>Test: {testDetails?.testName || "Loading..."}</CardDescription></CardHeader>
          <CardContent>
  ...
  if (!currentQuestion && !isLoadingPage && testSessionState === 'in_progress') { return ( <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/90 p-4 text-white"> <Card className="w-full max-w-lg text-center shadow-xl bg-background text-foreground"> <CardHeader> <AlertCircle className="mx-auto h-12 w-12 text-muted-foreground" /> <CardTitle>No Questions</CardTitle> </CardHeader> <CardContent><p className="text-muted-foreground">No questions available for this test.</p></CardContent> <CardFooter><Button onClick={() => router.back()} variant="outline" className="w-full">Go Back</Button></CardFooter> </Card> </div> );}
