...
export default function TeacherTestStudentResultPage() {
  ...
  const fetchResultData = useCallback(async (isMountedGetter: () => boolean = () => true) => {
    ...
    try {
      ...
      if (isMountedGetter()) {
        const correct = fetchedAttemptData.correct_answers_count ?? fetchedAttemptData.score ?? 0;
  ...
  const fetchUserNotebooks = useCallback(async () => {
    ...
    try {
      const records = await pb.collection('student_bookmarks').getFullList<StudentBookmark>({ filter: `user = "${user.id}"`, sort: '-updated', '$autoCancel': false, });
  ...
  const handleSaveToNotebooks = async () => {
    ...
    for (const notebookId of Array.from(selectedNotebookIdsInModal)) {
      try {
        const notebook = await pb.collection('student_bookmarks').getOne<StudentBookmark>(notebookId, {'$autoCancel': false,});
        ...
        if (!existingQuestions.includes(currentReviewQuestion.id)) {
          await pb.collection('student_bookmarks').update(notebookId, { "questions+": currentReviewQuestion.id }, {'$autoCancel': false,});
        }
        successCount++;
      } catch (err) {
        errorCount++;
  ...
  const handleReportSubmit = async (data: ReportInput) => {
    ...
    try {
      await pb.collection('report_by_students').create({ user: user.id, question: reportingQuestionId, test_in_which_report_is_made: attemptData.teacher_test, TypeOfError: data.TypeOfError, Please_write_your_report_here: data.Please_write_your_report_here || null, }, {'$autoCancel': false,});
